...
execute store result storage gammahollow:data current_damage int 1 run random value 1..10

# 2. CALL THE MACRO (The critical 'execute as @s' fix)
# This ensures that inside the macro, @s refers to the entity being damaged.
execute as @s run function gammahollow:apply_damage_macro with storage gammahollow:data
