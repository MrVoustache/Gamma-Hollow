...
execute as @e[predicate=gamma:exposed_to_cosmic_rays, tag=!has_timer] run scoreboard players set @s radiation_timer 0

# 4. The logic from before: hit those at 0 and start timer
execute as @e[predicate=gamma:exposed_to_cosmic_rays, scores={radiation_timer=..0}] unless entity @s[type=player, gamemode=creative] unless entity @s[type=player, gamemode=spectator] at @s run function gamma:trigger_damage
