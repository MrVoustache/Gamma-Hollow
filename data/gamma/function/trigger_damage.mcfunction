...
execute store result storage gamma:data current_damage int 1 run random value 1..10

# 2. CALL THE MACRO (The critical 'execute as @s' fix)
# This ensures that inside the macro, @s refers to the entity being damaged.
execute as @s run function gamma:apply_damage_macro with storage gamma:data
